...
class AboutIteration < Neo::Koan # rubocop:disable Style/Documentation
  # -- An Aside ------------------------------------------------------
  ...
  def test_break_works_with_each_style_iterations
    ...
    sum = 0
    array.each do |item|
      break if item > 3 # rubocop:disable Layout/EmptyLineAfterGuardClause
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_select_selects_certain_items_from_an_array
    ...
    even_numbers = array.select { |item| item.even? } # rubocop:disable Style/SymbolProc
